...
// Black icons (no scroll/hover check)
const UserIcon = ({ className }: { className?: string }) => (
  <svg className={className} fill="none" width="24" viewBox="0 0 24 24">
    <path
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 pt-4">
        ...
        >
          ...
          <div className="flex items-center space-x-6 absolute right-0">
            <motion.button onClick={handleUserClick} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
              <UserIcon className="h-6 w-6" />
            </motion.button>

            <motion.button onClick={handleSearchClick} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
          ...
          {Object.keys(categories).map((category, index) => (
            ...
            >
              <motion.button
                ...
                className="text-black hover:text-gray-600 text-sm font-extralight tracking-wide py-2 px-1 relative block transition-colors"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                {category}
                <motion.div
                  className="absolute top-full left-0 h-0.5 mt-4 z-50 bg-black"
                  initial={{ width: 0 }}
                  animate={{
                    width:
                      hoveredCategory === category && category === "BRAND"
                        ? "800px"
                    ...
                    <div className="grid grid-cols-4 gap-0 w-[800px] p-6">
                      {categories["BRAND"].map((brand) => (
                        <motion.a
                          ...
                          className="block text-xs text-gray-600 hover:text-black transition-colors tracking-wider py-1 cursor-pointer uppercase"

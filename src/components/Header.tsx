...
//     >
//       <AnnouncementBar className={`${isScrolled ? 'hidden' : 'block'}`} />
//       <div className="mx-auto px-4 sm:px-6 lg:px-8 sm:pt-4 py-1 ">
...
//             transition={{ duration: 0.5 }}
//           >
//             <div className='h-8 sm:h-12 w-fit max-w-24 my-3.5 flex sm:w-full'>
//               <Image
//                 src="/assets/images/logo.png"
//                 alt="House of Perfumes Logo"
//                 width={80}
...
//           <div className="flex items-center justify-end space-x-3.5 sm:space-x-6 w-1/3">
//             <motion.button 
//               onClick={handleUserClick}
//               className="hidden sm:block transition-colors duration-300 cursor-pointer"
//               whileHover={{ scale: 1.1 }}
//               whileTap={{ scale: 0.95 }}
//               initial={{ opacity: 0, x: 20 }}
//               animate={{ opacity: 1, x: 0 }}
...
//               transition={{ delay: 0.3 }}
//             >
//               <SearchIcon className="h-5.5 w-5.5 sm:h-6 sm:w-6" isScrolled={isScrolled} isHovered={isHovered} />
//             </motion.button>
...
//               transition={{ delay: 0.4 }}
//             >
//               <ShoppingBagIcon className="h-5.5 w-5.5 sm:h-6 sm:w-6" isScrolled={isScrolled} isHovered={isHovered} />
//               {totalItems > 0 && (
//                 <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
//                   {totalItems}
//                 </span>
...
//         <motion.nav 
//           className="hidden sm:flex justify-center space-x-8 pb-4"
//           initial={{ opacity: 0, y: 20 }}
//           animate={{ opacity: 1, y: 0 }}
//           transition={{ duration: 0.6, delay: 0.3 }}
//         >
...
import AnnouncementBar from './AnnouncementBar';
import { fetchCart, removeFromCart, updateQuantity } from '@/slices/cartSlice';
import { RxHamburgerMenu as MenuIcon, RxCross1 as CloseIcon } from "react-icons/rx";
import { AppDispatch, RootState } from '@/store/store';
import { useDispatch, useSelector } from 'react-redux';
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8 sm:pt-4 py-1 ">
          ...
          >
            <div className='w-1/3 flex justify-start'>
              <button onClick={toggleMobileMenu} className='text-lg font-semibold sm:hidden'>
                <MenuIcon className={`h-6 w-6 ${isScrolled || isHovered ? 'text-black/80' : 'text-white'}`} />
              </button>
            </div>
          ...
          >
            {Object.keys(categories).map((category, index) => (
              ...
              >
                <motion.button
                  onClick={() => {
                    if (category === 'ABOUT US') handleNavClick('/about');
                    if (category === 'CONTACT US') handleNavClick('/contact');
                  }}
                  className={`${
                    isScrolled || isHovered 
                      ? 'text-black hover:text-gray-600' 
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            ...
            />
            <motion.div
              className="fixed top-0 left-0 h-full w-80 bg-white shadow-xl z-50 p-8"
              initial={{ x: '-100%' }}
              animate={{ x: 0 }}
              exit={{ x: '-100%' }}
              transition={{ type: 'spring', damping: 25, stiffness: 200 }}
            >
              ...
              <nav className="flex flex-col space-y-6">
                <a onClick={() => handleNavClick('/')} className="text-lg font-light tracking-wider uppercase cursor-pointer">Home</a>
                <a onClick={() => handleNavClick('/collections')} className="text-lg font-light tracking-wider uppercase cursor-pointer">Collections</a>
                <a onClick={() => handleNavClick('/about')} className="text-lg font-light tracking-wider uppercase cursor-pointer">About Us</a>
                <a onClick={() => handleNavClick('/contact')} className="text-lg font-light tracking-wider uppercase cursor-pointer">Contact Us</a>
                <hr/>
                <a onClick={handleUserClick} className="text-lg font-light tracking-wider uppercase cursor-pointer">Login</a>
              </nav>
            </motion.div>
          </>
        )}
